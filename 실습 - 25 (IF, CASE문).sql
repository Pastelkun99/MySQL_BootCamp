...
-- ifProc()을 호출한다.
 ...
 --  입사한지 5년이 지났는지 직접 확인하는 것을 프로그래밍 해보자.
 -- 먼저 변수를 date타입 2개(현재, 입사년도)를 선언하고, 날짜계산 하기위해 days를 int형으로 선언한다.
 -- select hire_date into hiredate는 hire_date를 hiredate에 대입하라는 쿼리문이다.
 -- 그리고 hiredate라는 변수를 이용해서 날짜를 계산하면 된다.
...
begin
    ...
    select concat('취득 점수 -->', point) as '점수', concat('학점 --> ', credit) as '취득학점';
...
-- 고객 이름이 없으니 조인을 이용해 usertbl과 엮도록 하자
-- 결과를 보면 총구매액별로 내림차순 정렬이 되었고, 구매하지 않는 사람도 있다.
...
-- select문 안에 하나의 필드처럼 고객등급을 case when then 문으로 설정을 하는 쿼리문이다.
select U.userid, U.name, sum(price*amount) as '총구매액',
	case
