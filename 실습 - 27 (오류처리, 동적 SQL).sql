...
-- 아래 내용은 직접 우리가 오류코드를 처리해주는 구문은 procedure로 만든것이다.
...
begin
	...
	-- 1146은 테이블이 없을때 mysql에서 직접 발생시키는 오류코드 번호이고 sqlstate '42502'도 역시
    -- 테이블이 없는 sql상태를 나타내는 구문이다. 오류코드나 오류상태를 ppt에서 말해준 사이트에서
    -- 찾아서 넣어주면 여러분들도 오류코드처리구문을 만들수가 있다.
    -- 일반적으로 1106이나 이런 숫자가 나오는 것보다 사용자가 알아보기 쉽게 오류처리구문을 넣어주면
    -- 디버깅할때 상당한 도움이 될 것이다.
...
begin
    ...
    begin
        ...
        select '기본키는 중복될 수 없네요. 작업을 취소시킵니다' as '메시지';
        rollback;
	end;
    -- usertbl에서는 userid가 기본키이므로 분명 오류가 발생할 것이다.
...
-- 여러번 실행해보자.
-- 현재 날짜와 시간을 curdate변수에 넣는다. 즉 실행시점에 날짜와 시간이 들어가는 것이다.
-- ex) 회원가입이나, 구매시점, 등하교 등 어느 시점을 기록하고 싶을 때 종종쓰인다.
