use tabledb;
-- 아래 prodtbl 테이블을 만들었다. 하지만 제약 조건을 주지 않았다.
...
-- 그래서 아래와 같이 테이블을 수정(alter table table명) 하는 구문을 넣어줄 수가 있다.
-- 근데 여기서는 PK를 직접 이름을 주어서 만들었고, 아울러 2개의 필드를 합쳐서 하나의 PK로 설정한 것이다.
-- 어느 필드가 PK가 될 수 있겠는가?
...
alter table prodtbl
    ...
    primary key(prodcode, prodid);
-- 하기 2개 명령은 비슷하다. 테이블 구조를 보면 prodcode와 prodid가 PK로 설정되어 있는 것을 확인할 수 있다.
desc prodtbl;
